...
var githubProxyURL = "https://gprox-galaxy.github.io/PROXYIP.txt";
    ...
    try {
        ...
        if (response.ok) {
            ...
            const fetchedIPs = text.split('\n')
                ...
                .filter(line => /^[a-z0-9.-]+$/i.test(line) && line.length <= 253);
    ...
    (async () => {
        ...
        try {
            while (true) {
                ...
                if (value.byteLength > 32 * 1024 * 1024) throw new Error("gRPC frame exceeds limit");
                ...
                await handleTCPOutBound(remoteSocketWrapper, addressRemote, portRemote, rawClientData, grpcClient, responseHeader, log);
            }
        } catch (err) {
...
async function proxyOverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            if (typeof chunk === "string") throw new Error("Text WebSocket frames are not supported");
            if (chunk instanceof Blob) chunk = await chunk.arrayBuffer();
            const bytes = chunk instanceof ArrayBuffer ? new Uint8Array(chunk) : new Uint8Array(chunk);
            if (isDns && udpStreamWrite) {
                return udpStreamWrite(bytes);
            }
            if (remoteSocketWrapper.value) {
            ...
            let result = processVlessHeader(bytes, userID);
            ...
            const rawClientData = bytes.slice(rawDataIndex);
...
function processVlessHeader(vlessBuffer, userID2) {
    const bytes = vlessBuffer instanceof Uint8Array ? vlessBuffer : new Uint8Array(vlessBuffer);
    if (bytes.byteLength < 24) {
        return { hasError: true, message: "Invalid VLESS data" };
    }

    const version = bytes.slice(0, 1);
    const slicedBuffer = bytes.slice(1, 17);
    ...
    if (commandIndex >= bytes.length) return { hasError: true, message: "Truncated VLESS command" };
    ...
    if (portIndex + 3 > bytes.length) return { hasError: true, message: "Truncated VLESS port/address" };
    const portRemote = new DataView(bytes.buffer, bytes.byteOffset + portIndex, 2).getUint16(0);
    ...
    switch (addressType) {
        case 1:
            addressLength = 4;
            if (addressValueIndex + addressLength > bytes.length) return { hasError: true, message: "Truncated IPv4 address" };
            addressValue = bytes.slice(addressValueIndex, addressValueIndex + addressLength).join(".");
            break;
        case 2:
            if (addressValueIndex >= bytes.length) return { hasError: true, message: "Truncated domain length" };
            addressLength = bytes[addressValueIndex];
            addressValueIndex += 1;
            if (addressValueIndex + addressLength > bytes.length) return { hasError: true, message: "Truncated domain address" };
            addressValue = new TextDecoder().decode(bytes.slice(addressValueIndex, addressValueIndex + addressLength));
            break;
        case 3:
            addressLength = 16;
            if (addressValueIndex + addressLength > bytes.length) return { hasError: true, message: "Truncated IPv6 address" };
            const dataView = new DataView(bytes.buffer, bytes.byteOffset + addressValueIndex, addressLength);
            const ipv6 = [];
            for (let i = 0; i < 8; i++) {
    ...
    if (portRemote < 1 || portRemote > 65535) {
